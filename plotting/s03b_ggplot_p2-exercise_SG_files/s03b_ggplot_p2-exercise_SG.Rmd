...
gapminder %>% 
  group_by(country) %>% 
  ggplot(aes(group=country, year, lifeExp, alpha=0.01)) +
  geom_line()
```
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  scale_x_log10( "GDP Per Cap", labels =scales::comma_format())+
...
gapminder %>% 
  ...
  ggplot( shape=21, colour="red", fill="black", aes(gdpPercap, lifeExp,)) +
...
gapminder %>% 
  filter(continent == "Americas") %>% 
  ggplot(aes(country, lifeExp)) +
  ggridges::geom_density_ridges (mapping = aes(country, lifeExp))
  # geom_point() +
  # geom_boxplot()
```
