...
```{r}
#When pulling column headings, R is case sensitive
...
#Not as good way but still works... pulls first 4 columns
#Not as good because if column order changes you mess up
...
```{r}
#This code tells R to use Gapminder data but since no columns are specified pull no columns.
select(gapminder)

#Pull everything in gapminder except lifeExp
...
#Save rename or reorganizing changes to a new table
gapminder_new <- rename(gapminder, cont = continent)
```

Doing everything with mtcars instead of gapminder
...
```{r}
#The + sign below is an example of a %>%. It can go between what it is trying to add or before
2+2
...
```{r}
if (6==5) 'five equals five' else 'does not equal'
```
```{r}
1:10 < c(2,6)
```
...
```{r}
#Filter Gapminder by population >100M and country in vector consisting of brazil or china
gapminder %>%
  filter(pop>100000000, country %in% c("Brazil", "China"))

#Do not use country == c(...) because it will check each row alternating for brazil and china

#%in% compares all values to whats in c to see if actual value matches anything in c
```
...
#Replace bangladesh life expectancy to NA
gapminder %>%
  mutate(lifeExp = if_else(country=='Bangladesh', NA_real_, lifeExp))%>% filter(country=='Bangladesh')
```
