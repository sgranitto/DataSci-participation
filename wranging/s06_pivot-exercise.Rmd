...
Yes, the data is in tidy format. 
...
```{r}
(guest_long <- guest %>% 
  pivot_longer(cols      = contains("attendance") | contains("meal"), 
               names_to  = "event" ,
               values_to = "status"))
```
...
guest_long %>% 
  group_by(party) %>% 
  summarize(all_pending = all(status == "PENDING")) 
```
...
guest_long %>% 
  group_by(party) %>% 
  filter( event == "attendance_wedding") %>% 
  summarize(pending_wedding = all(status == "PENDING"))
```
...
guest_long %>% 
  pivot_wider(id_cols     = c(-event, -status),
              names_from  = event, 
              values_from = status)
```
