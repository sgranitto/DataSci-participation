...
mtcars_plot <- ggplot(mtcars, aes(hp, wt)) + 
    geom_point()
ggsave(here::here("mtcars_plot.svg"), mtcars_plot)
```
...
This installs packages on guide 11 used to change aesthetics
```{r}
...
```{r}
#I was not in class due to corona virus pandemic and I do not see notes in the guide for Rcolorbrewer. I was able to install the package, and view all color palettes, but not sure how to make this work.
p_scales +
    scale_x_log10(labels=dollar_format()) +
    scale_colour_viridis_c_RCOLORBREWER(
        trans   = "log10",
        breaks  = 10^(1:10),
        labels  = comma_format(),
        palette = option
...
iris
library(ggplot2)
#The ggplot would not load even without my added theme line. Not sure why
...
#Error: package or namespace load failed for ‘tidyverse’ in get(method, envir = home): lazy-load database 'C:/Users/sgranitto/Documents/R/win-library/3.6/ggplot2/R/ggplot2.rdb' is corrupt

#I have tried reinstalling tidyverse and ggplot2 and keep getting the error. Ive gone to the directory and removed the tidyverse and ggplot packages and still get the error after reinstalling
```
